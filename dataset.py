...
def parse_paired_tfrecord_tf(record):
    features = tf.parse_single_example(record, features={
        'shape_img': tf.FixedLenFeature([3], tf.int64),
        'data_img': tf.FixedLenFeature([], tf.string),
        'shape_cap': tf.FixedLenFeature([3], tf.int64),
        'data_cap': tf.FixedLenFeature([], tf.string),
    })
    data_img = tf.decode_raw(features['data_img'], tf.uint8)
    data_img = tf.reshape(data_img, features['shape_img'])
    data_cap = tf.decode_raw(features['data_cap'], tf.uint8)
    data_cap = tf.reshape(data_cap, features['shape_cap'])
    return data_img, data_cap

def parse_paired_tfrecord_np(record):
    ex = tf.train.Example()
    ex.ParseFromString(record)
    shape_img = ex.features.feature['shape_img'].int64_list.value
    data_img = ex.features.feature['data_img'].bytes_list.value[0]
    data_img = np.fromstring(data_img, np.uint8).reshape(shape_img)
    data_cap = ex.features.feature['data_cap'].bytes_list.value[0]
    shape_cap = ex.features.feature['shape_cap'].int64_list.value
    data_cap = np.fromstring(data_cap, np.uint8).reshape(shape_cap)
    return data_img, data_cap
...
class TFRecordDataset:
    def __init__(self,
        ...
        self.resolution = resolution if resolution is not None else max_shape[2]
        self.resolution_log2 = int(np.log2(self.resolution))
        resolution2 = resolution if resolution is not None else max_shape[1]
        self.shape = [max_shape[0], resolution2, self.resolution]
        # tfr_lods = [self.resolution_log2 - int(np.log2(shape[1])) for shape in tfr_shapes]
        tfr_lods = [self.resolution_log2 - int(np.log2(shape[2])) for shape in tfr_shapes]
...
#----------------------------------------------------------------------------
# Dataset class that loads paired data from tfrecords files.

class TFRecordPairedDataset:
    def __init__(self,
        tfrecord_dir,               # Directory containing a collection of tfrecords files.
        resolution      = None,     # Dataset resolution, None = autodetect.
        label_file      = None,     # Relative path of the labels file, None = autodetect.
        ...
        self.tfrecord_dir       = tfrecord_dir
        self.resolution_img     = None
        self.resolution_cap     = None
        self.resolution_log2    = None
        self.shape              = []        # [channel, height, width]
        self.shape_img          = []        # [channel, height, width]
        self.shape_cap          = []        # [channel, height, width]
        self.dtype              = 'uint8'
        self.dynamic_range      = [0, 255]
        self.label_file         = label_file
        self.label_size         = None      # [component]
        ...
        for tfr_file in tfr_files:
            tfr_opt = tf.python_io.TFRecordOptions(tf.python_io.TFRecordCompressionType.NONE)
            for record in tf.python_io.tf_record_iterator(tfr_file, tfr_opt):
                data_img, data_cap = parse_paired_tfrecord_np(record)
                tfr_shapes_img.append(data_img.shape)
                tfr_shapes_cap.append(data_cap.shape)
                break
        ...
        # Determine shape and resolution.
        max_shape_img = max(tfr_shapes_img, key=lambda shape: np.prod(shape))
        max_shape_cap = max(tfr_shapes_cap, key=lambda shape: np.prod(shape))
        # self.resolution = resolution if resolution is not None else max_shape[1]
        self.resolution_img = resolution if resolution is not None else max_shape_img[2]
        self.resolution_cap = resolution if resolution is not None else max_shape_cap[2]
        self.resolution_log2 = int(np.log2(self.resolution_img))
        resolution_cap2 = resolution if resolution is not None else max_shape_cap[1]
        self.shape_img = [max_shape_img[0], self.resolution_img, self.resolution_img]
        self.shape_cap = [max_shape_cap[0], resolution_cap2, self.resolution_cap]
        self.shape = self.shape_img
        # tfr_lods = [self.resolution_log2 - int(np.log2(shape[1])) for shape in tfr_shapes]
        tfr_lods = [self.resolution_log2 - int(np.log2(shape[2])) for shape in tfr_shapes_img]
        assert all(shape[0] == max_shape_img[0] for shape in tfr_shapes_img)
        ...
        with tf.name_scope('Dataset'), tf.device('/cpu:0'):
            ...
            tfutil.set_vars({self._tf_labels_var: self._np_labels})
            self._tf_labels_dataset = tf.data.Dataset.from_tensor_slices(self._tf_labels_var)
            for tfr_file, tfr_shape_img, tfr_shape_cap, tfr_lod in zip(tfr_files, tfr_shapes_img, tfr_shapes_cap, tfr_lods):
                if tfr_lod < 0:
                    continue
                dset = tf.data.TFRecordDataset(tfr_file, compression_type='', buffer_size=buffer_mb<<20)
